                                    ...
                                    <nav aria-label="..." style="text-align:center;">
                                        <ul class="pagination">
                                            <li>
                                                ...
                                                <a href="javascript:ajaxsp
                                                lit(${info.prePage})" aria-label="Previous">
    ...
    <script type="text/javascript">
                ...
                if (confirm("您确定删除" + cks.length + "条商品吗？")) {
                    ...
                    $.ajax({
                        ...
                        success: function (msg) {
                            alert(msg)
                            $("#table").load("http://localhost/SSMMImi_war/admin/product.jsp #table");
                        }
        ...
        function del(pid,page) {
            if (confirm("确定删除吗")) {
                ...
                var Iprice = $("#lprice").val();
                var hprice = $("#hprice").val();
                $.ajax({
                    url: "${pageContext.request.contextPath}/prod/delete.action",
                    data: {"pid": pid,"page": page, "pname": pname, "typeid": typeid, "Iprice": Iprice, "hprice": hprice},
                    type: "post",
                    dataType: "text",
                    success: function (msg) {
                        alert(msg)
                ...
                ${pageContext.request.contextPath}/prod/delete.action?pid=" + pid;*/
            }
        }
        ...
        function one(pid,page) {
            ...
            var Iprice = $("#lprice").val();
            var hprice = $("#hprice").val();
            //不适用ajax，直接拼接上传当前页面的信息 "?pid=" + pid +
            var str = "?pid="+ pid +"&pname=" + pname + "&typeid=" + typeid + "&Iprice=" + Iprice + "&hprice=" + hprice+ "&page=" + page;
            location.href = "${pageContext.request.contextPath}/prod/one.action"+str;
            //location.href = "${pageContext.request.contextPath}/prod/one.action?pid=" + pid + "&page=" + page;
        }
    </script>
    <!--分页的AJAX实现-->
    <script type="text/javascript">
        function ajaxsplit(page) {
            ...
            $.ajax({
                ...
                success: function () {
                    //重新加载分页显示的组件table
                    //location.href---->http://localhost:80/admin/login.action
                    /*注意自己端口号是不是这个*/
                    $("#table").load("http://localhost/SSMMImi_war/admin/product.jsp #table");
                }
            })
